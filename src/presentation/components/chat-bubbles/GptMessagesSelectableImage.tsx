...
export const GptMessagesSeletableImage = ({
  imageUrl,
  onImageSelected,
}: Props) => {
  ...
  useEffect(() => {
    ...
    // Estoy manteniendo la imagen en memoria y manteniendo el contexto en el que voy a dibujar
  ...
  const onMouseMove = (
    ...
    ctx.clearRect(coords.x, coords.y, width, height); // Para que borre el rectangulo
  };

  // Este es para reestablecer todo basado en la imagen original
  const resetCanvas = () => {
    const ctx = canvasRef.current!.getContext("2d")!;
    ctx.clearRect(0, 0, canvasRef.current!.width, canvasRef.current!.height);
    ctx.drawImage(
